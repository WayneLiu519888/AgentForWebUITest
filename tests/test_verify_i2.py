...
def test_verify_i2():
    """验证策略解析、版本同步、用例生成和JSON序列化。"""
    print("=" * 60)
    print("  AgentForWebUITest 最终验证")
    print("=" * 60)
    ...
    # 2. Agent版本（动态读取，避免硬编码阻塞版本升级）
    print("\n2. Agent版本...")
    agent = WebUITestAgent()
    EXPECTED_VERSION = WebUITestAgent.VERSION
    EXPECTED_ITERATIONS = WebUITestAgent.ITERATIONS
    assert agent.VERSION == EXPECTED_VERSION, f"VERSION不匹配: {agent.VERSION} vs {EXPECTED_VERSION}"
    assert agent.ITERATIONS == EXPECTED_ITERATIONS, f"ITERATIONS不匹配: {agent.ITERATIONS} vs {EXPECTED_ITERATIONS}"
